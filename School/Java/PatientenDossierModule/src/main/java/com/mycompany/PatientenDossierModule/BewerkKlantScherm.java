 ...
 * Bestandsnaam: BewerkKlantScherm.java
 */

public class BewerkKlantScherm 
{
	...
	private int huidigeKlantNr;
	public Klant huidigeKlant = new Klant();
	
	public BewerkKlantScherm()
	{
		huidigeKlantNr = 0;
	...
	public int klantBewerken()
	{
		Patiënt huidigePatiënt = selectieTabel.getSelectionModel().getSelectedItem();
		
		huidigeKlant.setGeslacht((rbMan.isSelected() ? false : true));
		huidigeKlant.setGeboorteDatum(txtfGeboorteDatum.getText());
		huidigeKlant.setWoonAdres(txtArWoonAdres.getText());
		huidigeKlant.setTelefoonNummer(txtfTelefoonNr.getText());
		huidigeKlant.setEmailAdres(txtfEmailAdres.getText());
		huidigeKlant.setPatiëntNummer(huidigePatiënt.getPatiëntNummer());
		huidigeKlant.setPatiëntNaam(huidigePatiënt.getNaam());
		
		huidigeKlant.klantBewerken();
		
		return huidigeKlantNr;
	}
	...
	{
		huidigeKlant = selectedKlant;
		//txtfNaam.setText(selectedKlant.getNaam());
		// geslacht: false = Man; true = vrouw;
		if(selectedKlant.getGeslacht()) 
		{
		...
		txtfGeboorteDatum.setText(selectedKlant.getGeboorteDatum());
		txtfTelefoonNr.setText(selectedKlant.getTelefoonNummer());
		txtfEmailAdres.setText(selectedKlant.getEmailAdres());
		txtArWoonAdres.setText(selectedKlant.getWoonAdres());
		selectieTabel.getSelectionModel().select(selectedKlant);		
	}
}
