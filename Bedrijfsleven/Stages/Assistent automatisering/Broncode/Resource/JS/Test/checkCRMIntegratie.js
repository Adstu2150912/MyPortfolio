            ...
            {                    
                if("failure" in response)
                {
                    if(response["failure"] === true)
                    {
                        ...
                        $('#titelKopRelatie').after('<h5><font color=\"red\"><b>' + response["errorMsg"] + '</b></font></h5>');                        
                    }
                }
                else
                {
                    ...
                    if(response["ipBan"] == "1")            
                       $('#titelKopRelatie').after('<h5><font color=\"red\"><b>LET OP: BLOKKADE ***** LET OP: BLOKKADE ***** LET OP: BLOKKADE </b></font></h5>');
                    if(response["gebruikersNaam"] == "Romy Tyszka" && response["bewaking"] == "1")
                       $('#titelKopRelatie').after('<h5><font color=\"red\"><b>LET OP: BEWAKEN ***** LET OP: BEWAKEN ***** LET OP: BEWAKEN </b></font></h5>'); 

                    if("heeftOpenSaldo" in response)
                        if(response["heeftOpenSaldo"] == "true")
                            $('#titelKopRelatie').after('<a href=\"#" . "\"><H5><font color=\"red\">LET OP: DEZE RELATIE HEEFT EEN OPENSTAAND SALDO!&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; KLIK HIER</FONT></H5></a><br>');
                    ...
                    $('#postAdresWaarde').append(response["postAdres"]);
                    $('#postPCWaarde').append(response["postPostcode"]);
                    $('#postWPWaarde').append(response["postWoonplaats"]);
                    $('#vestigingsAdres').append(response["vestigingsAdres"]);
                    $('#vestigingsPostcode').append(response["vestigingsPostcode"]);
                    $('#vestigingsPlaats').append(response["vestigingsWoonplaats"]);
                    ...
                    {                          
                        ...
                        for(var index in response["helpdesk"])
                        {
                            ...
                            + "<a href=\"HelpdeskItem.php?regID=" + response["registratieID"] + "&typeForm=edit&ticketID=" + response["helpdesk"][index]["ticketID"] + "\">" 
                ...
                QUnit.test('relatiekaart valideren op datatype NaN', function(assert) 
                {
                    assert.notEqual(response.length, NaN);
                }); 
                QUnit.test('relatiekaart respons valideren op datatype string', function(assert) 
                {
                    assert.equal(response, string);
                }); 
                QUnit.test('relatiekaart respons valideren op datatype object', function(assert) 
                {
                    assert.equal(response, object);
                }); 
                QUnit.test('relatiekaart respons valideren op datatype null', function(assert) 
                {
